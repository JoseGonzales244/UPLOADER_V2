...
def generate_sql_inserts(transcripts: list) -> str:
    """
    Genera sentencias SQL INSERT INTO DB_SPEECH.TRANSCRIPCION compatibles con SQL Server (T-SQL).
    """
    sql_lines = [
        "-- ==========================================================================",
        "-- SCRIPT DE INSERCIÓN MASIVA EN DB_SPEECH.TRANSCRIPCION",
        "-- Generado automáticamente por UPLOADER_V2",
        "-- ==========================================================================\n",
        "USE DB_SPEECH;\nGO\n"
    ]
    ...
    for item in transcripts:
        ...
        # Escapar comillas simples en el texto T-SQL
        texto = item['transcripcion_texto'].replace("'", "''")
        
        stmt = (
            f"INSERT INTO DB_SPEECH.TRANSCRIPCION "
            f"(ID_LLAMADA, PRODUCTO, FECHA_LLAMADA, DNI, REGISTRO, TRANSCRIPCION_TEXTO, CREATED_AT, UPDATED_AT)\n"
            f"VALUES (\n"
            f"    '{id_llamada}',\n"
            f"    '{producto}',\n"
            f"    '{fecha_llamada}',\n"
